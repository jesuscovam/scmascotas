<script lang="ts">
	...
	const activeSightings = $derived(
		data.sightings.filter((s: { status: string }) => s.status !== 'archived')
	);
	const archivedSightings = $derived(
		data.sightings.filter((s: { status: string }) => s.status === 'archived')
	);
	const shownSightings = $derived(activeTab === 'active' ? activeSightings : archivedSightings);

	const openCount = $derived(activeSightings.filter((s: { status: string }) => s.status === 'open').length);
	const resolvedCount = $derived(activeSightings.filter((s: { status: string }) => s.status === 'resolved').length);
	...
	async function handleArchive(id: string, archive: boolean) {
		...
		archivingId = id;
		try {
			const res = await fetch(`/api/spotted-pets/${id}/archive`, {
				method: 'PATCH',
...
<div class="min-h-[calc(100vh-4rem)] bg-warm-50 dark:bg-warm-900 pb-20">
					...
					{#if activeSightings.length > 0}
						<div class="flex items-center gap-3 mt-3 flex-wrap">
							<span class="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-warm-100 dark:bg-warm-700 text-warm-600 dark:text-warm-300 border border-warm-200 dark:border-warm-600">
								{activeSightings.length} reporte{activeSightings.length !== 1 ? 's' : ''}
							</span>
							{#if openCount > 0}
	...
	<div class="max-w-5xl mx-auto px-4 py-8 flex flex-col gap-6">
		...
		<!-- Tab switcher -->
		{#if data.sightings.length > 0 || archivedSightings.length > 0}
			...
			{#if activeTab === 'archived'}
				...
				<div class="flex flex-col items-center justify-center py-16 text-center rounded-2xl border-2 border-dashed border-warm-200 dark:border-warm-700 bg-white dark:bg-warm-800/40">
					...
					<p class="text-sm text-warm-400 dark:text-warm-500 max-w-xs leading-relaxed">
						Los avistamientos que archives aparecerán aquí. Puedes restaurarlos en cualquier momento.
					</p>
				</div>
			{:else}
		...
		{:else}
			<div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
				{#each shownSightings as s (s.id)}
					<div
						class="group relative flex flex-col bg-white dark:bg-warm-800 border rounded-xl shadow-sm overflow-hidden sighting-card
							{activeTab === 'archived'
								? 'border-warm-200 dark:border-warm-700 opacity-75 hover:opacity-100'
								: 'border-warm-200 dark:border-warm-700 hover:shadow-md hover:border-teal-300 dark:hover:border-teal-700'}
							{archivingId === s.id ? 'opacity-40 pointer-events-none' : ''}"
					>
						<!-- Full-card link -->
						<a
							href="/avistamientos/{s.slug}"
							class="absolute inset-0 z-0 rounded-xl focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-teal-500 focus-visible:outline-none"
							aria-label="Ver avistamiento {typeLabel[s.type] ?? s.type}"
						></a>
						...
						<div class="relative z-10 flex flex-col flex-1 pointer-events-none">
							...
							<div class="h-0.5 w-full
								{s.status === 'resolved' ? 'bg-emerald-400 dark:bg-emerald-500' :
								 s.status === 'archived' ? 'bg-warm-200 dark:bg-warm-700' :
							...
							<div class="p-5 flex flex-col gap-3 flex-1">
								<div class="flex items-start justify-between gap-2">
									<div class="flex items-center gap-2.5 min-w-0">
										<div class="w-9 h-9 rounded-lg flex items-center justify-center text-lg shrink-0 select-none
											{activeTab === 'archived' ? 'bg-warm-100 dark:bg-warm-700' : 'bg-teal-50 dark:bg-teal-900/30'}">
											{typeEmoji[s.type] ?? '🐾'}
										</div>
										<div class="min-w-0">
											<p class="font-display font-semibold leading-tight
												{activeTab === 'archived' ? 'text-warm-500 dark:text-warm-400' : 'text-warm-900 dark:text-warm-50 group-hover:text-teal-800 dark:group-hover:text-teal-400'} transition-colors">
									...
									<span class="shrink-0 text-[11px] font-medium px-2 py-0.5 rounded-full border
										{s.status === 'resolved'
											...
											: s.status === 'archived'
											? 'bg-warm-100 dark:bg-warm-700 text-warm-500 dark:text-warm-400 border-warm-200 dark:border-warm-600'
											: 'bg-teal-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 border-teal-200 dark:border-teal-700'}">
										{s.status === 'resolved' ? 'Resuelto' : s.status === 'archived' ? 'Archivado' : 'Abierto'}
									</span>
								</div>
								...
								{#if s.matchedPetName && s.matchedPetSlug}
									<div class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-xs
										{activeTab === 'archived'
											? 'bg-warm-50 dark:bg-warm-800 border-warm-200 dark:border-warm-600'
											: 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800'}">
										<span class="text-base leading-none">🐾</span>
										<span class="font-medium truncate
											{activeTab === 'archived' ? 'text-warm-500 dark:text-warm-400' : 'text-amber-800 dark:text-amber-300'}">
											Vinculado a {s.matchedPetName}
										</span>
										<svg class="w-3 h-3 shrink-0 ml-auto {activeTab === 'archived' ? 'text-warm-400' : 'text-amber-500'}" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 6h8M6 2l4 4-4 4"/></svg>
									</div>
								{/if}
								...
								<div class="flex items-center justify-between mt-auto pt-1 border-t border-warm-100 dark:border-warm-700">
									...
									<span class="text-xs text-warm-300 dark:text-warm-600 flex items-center gap-0.5
										{activeTab === 'archived' ? '' : 'group-hover:text-teal-600 dark:group-hover:text-teal-500'} transition-colors">
										Ver
						...
						{#if activeTab === 'active'}
							<button
								...
								title="Archivar avistamiento"
								disabled={archivingId === s.id}
								aria-label="Archivar este avistamiento"
							>
								{#if archivingId === s.id}
						...
						{:else}
							<button
								class="absolute top-3 right-3 z-10 p-1.5 rounded-lg transition-all duration-150
									bg-warm-100 dark:bg-warm-700 hover:bg-teal-100 dark:hover:bg-teal-900/40
									text-warm-400 dark:text-warm-500 hover:text-teal-600 dark:hover:text-teal-400"
								onclick={() => handleArchive(s.id, false)}
								title="Restaurar avistamiento"
								disabled={archivingId === s.id}
								aria-label="Restaurar este avistamiento"
							>
								{#if archivingId === s.id}
									<svg class="w-3.5 h-3.5 animate-spin" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><circle cx="8" cy="8" r="6" stroke-opacity=".25"/><path d="M14 8a6 6 0 00-6-6"/></svg>
								{:else}
