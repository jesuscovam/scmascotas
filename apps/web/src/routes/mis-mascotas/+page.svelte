<script lang="ts">
	...
	const statusClasses: Record<string, string> = {
		...
		archived: 'bg-warm-100 dark:bg-warm-800 text-warm-500 dark:text-warm-400 border-warm-200 dark:border-warm-600'
	};

	const missingCount = $derived(data.pets.filter((p: { status: string }) => p.status === 'missing').length);
	const reunitedCount = $derived(data.pets.filter((p: { status: string }) => p.status === 'reunited').length);
					...
					{#if data.pets.length > 0}
						<div class="flex items-center gap-3 mt-3 flex-wrap">
							<span class="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-warm-100 dark:bg-warm-700 text-warm-600 dark:text-warm-300 border border-warm-200 dark:border-warm-600">
								{data.pets.length} reporte{data.pets.length !== 1 ? 's' : ''}
							</span>
							{#if missingCount > 0}
			...
			<div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
				{#each data.pets as pet (pet.id)}
					<a
						...
						class="group relative flex flex-col bg-white dark:bg-warm-800 border border-warm-200 dark:border-warm-700 rounded-xl shadow-sm hover:shadow-md hover:border-amber-300 dark:hover:border-amber-700 transition-all pet-card overflow-hidden"
					>
						<!-- Status accent bar -->
						...
						<div class="p-5 flex flex-col gap-3 flex-1">
							<div class="flex items-start justify-between gap-2">
								...
								<div class="flex items-center gap-2.5 min-w-0">
									<div class="w-9 h-9 rounded-lg bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center text-lg shrink-0 select-none">
										{typeEmoji[pet.type] ?? '🐾'}
									</div>
									<div class="min-w-0">
										<p class="font-display font-semibold text-warm-900 dark:text-warm-50 group-hover:text-amber-800 dark:group-hover:text-amber-400 transition-colors truncate leading-tight">
											{pet.name ?? 'Sin nombre'}
										</p>
								...
								<!-- Status badge -->
								<span class="shrink-0 text-[11px] font-medium px-2 py-0.5 rounded-full border {statusClasses[pet.status]}">
							...
							<div class="flex items-center justify-between mt-auto pt-1 border-t border-warm-100 dark:border-warm-700">
								...
								<span class="text-xs text-warm-300 dark:text-warm-600 group-hover:text-amber-600 dark:group-hover:text-amber-500 transition-colors flex items-center gap-0.5">
									Ver
