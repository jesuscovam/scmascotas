<script lang="ts">
	import { Button, Card, Input, Label, Separator, Spinner } from '@scmascotas/ui';
	import { signIn, signUp } from '$lib/auth-client';

	let { data } = $props();

	let name = $state('');
	let email = $state('');
	let password = $state('');
	let error = $state('');
	let loading = $state(false);
	let emailSent = $state(false);
	let sentToEmail = $state('');

	async function handleRegister(e: SubmitEvent) {
		e.preventDefault();
		error = '';
		loading = true;
		const result = await signUp.email({ name, email, password, callbackURL: '/mis-mascotas' });
		loading = false;
		if (result.error) {
			error = result.error.message ?? 'No se pudo crear la cuenta';
		} else {
			sentToEmail = email;
			emailSent = true;
		}
	}

	async function handleGoogle() {
		await signIn.social({ provider: 'google', callbackURL: '/mis-mascotas' });
	}
</script>

<svelte:head><title>Crear cuenta — SC Mascotas</title></svelte:head>

<div class="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-16 bg-warm-100 dark:bg-warm-900">
	<div class="w-full max-w-sm">
		<div class="text-center mb-8">
			<span class="text-4xl leading-none block">🐾</span>
			<h1 class="font-display text-2xl font-bold text-warm-900 dark:text-warm-50 mt-3 mb-1">
				Crea tu cuenta
			</h1>
			<p class="text-warm-500 dark:text-warm-400 text-sm">
				Guarda tus reportes y recibe actualizaciones
			</p>
		</div>

		{#if emailSent}
			<Card.Root class="shadow-sm border-warm-200 dark:border-warm-700 bg-white dark:bg-warm-800">
				<Card.Content class="pt-8 pb-8 px-6 flex flex-col items-center gap-4 text-center">
					<div class="w-12 h-12 rounded-full bg-brand-100 dark:bg-brand-900/40 flex items-center justify-center">
						<svg class="w-6 h-6 text-brand-700 dark:text-brand-400" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
						</svg>
					</div>
					<div>
						<h2 class="font-display font-bold text-warm-900 dark:text-warm-50 text-lg leading-snug">Revisa tu correo</h2>
						<p class="text-warm-500 dark:text-warm-400 text-sm mt-1.5">
							Enviamos un enlace de confirmación a<br />
							<span class="font-medium text-warm-700 dark:text-warm-200">{sentToEmail}</span>
						</p>
					</div>
					<p class="text-xs text-warm-400 dark:text-warm-500 leading-relaxed max-w-[260px]">
						Haz clic en el enlace del correo para activar tu cuenta. El enlace expira en 24 horas.
					</p>
				</Card.Content>
			</Card.Root>

			<p class="text-center text-sm text-warm-500 dark:text-warm-400 mt-6">
				¿No recibiste el correo?
				<button
					onclick={() => { emailSent = false; }}
					class="text-brand-800 dark:text-brand-400 hover:underline font-medium"
				>
					Intentar de nuevo
				</button>
			</p>
		{:else}
			<Card.Root class="shadow-sm border-warm-200 dark:border-warm-700 bg-white dark:bg-warm-800">
				<Card.Content class="pt-6 pb-6 px-6 flex flex-col gap-4">
					{#if error}
						<p class="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2">
							{error}
						</p>
					{/if}

					<form onsubmit={handleRegister} class="flex flex-col gap-4">
						<div class="flex flex-col gap-1.5">
							<Label.Label for="name">Nombre</Label.Label>
							<Input.Input
								id="name"
								type="text"
								placeholder="Tu nombre"
								bind:value={name}
								required
								autocomplete="name"
							/>
						</div>
						<div class="flex flex-col gap-1.5">
							<Label.Label for="email">Correo electrónico</Label.Label>
							<Input.Input
								id="email"
								type="email"
								placeholder="tu@correo.com"
								bind:value={email}
								required
								autocomplete="email"
							/>
						</div>
						<div class="flex flex-col gap-1.5">
							<Label.Label for="password">Contraseña</Label.Label>
							<Input.Input
								id="password"
								type="password"
								placeholder="Mínimo 8 caracteres"
								bind:value={password}
								required
								minlength={8}
								autocomplete="new-password"
							/>
						</div>
						<Button.Root type="submit" class="w-full gap-2" disabled={loading}>
							{#if loading}<Spinner class="size-4" />{/if}
							{loading ? 'Creando cuenta…' : 'Crear cuenta'}
						</Button.Root>
					</form>

					{#if data.hasGoogle}
						<div class="flex items-center gap-3">
							<Separator.Root class="flex-1" />
							<span class="text-xs text-warm-400 dark:text-warm-500">o</span>
							<Separator.Root class="flex-1" />
						</div>
						<Button.Root
							variant="outline"
							class="w-full gap-2"
							onclick={handleGoogle}
						>
							<svg viewBox="0 0 24 24" class="w-4 h-4 shrink-0" xmlns="http://www.w3.org/2000/svg">
								<path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
								<path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
								<path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
								<path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
							</svg>
							Continuar con Google
						</Button.Root>
					{/if}
				</Card.Content>
			</Card.Root>

			<p class="text-center text-sm text-warm-500 dark:text-warm-400 mt-6">
				¿Ya tienes cuenta?
				<a href="/login" class="text-brand-800 dark:text-brand-400 hover:underline font-medium">
					Inicia sesión
				</a>
			</p>
		{/if}
	</div>
</div>
