			...
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
				{#each data.pets as pet, i (pet.id)}
					<a
						...
						class="card-appear group bg-white dark:bg-warm-800 rounded-2xl border border-warm-200 dark:border-warm-700 border-t-4 {typeTopBorder[pet.type] ?? 'border-t-stone-400'} shadow-sm hover:shadow-md hover:scale-[1.015] transition-all overflow-hidden flex flex-col"
						style="animation-delay: {i * 35}ms"
					>
						<!-- Photo -->
						<div class="h-44 overflow-hidden bg-amber-50 dark:bg-warm-700 flex items-center justify-center text-6xl shrink-0">
							{#if pet.photoUrl}
								<img
									src={pet.photoUrl}
						...
						<div class="p-4 flex flex-col gap-3 flex-1">
							<div class="flex items-start justify-between gap-2">
								<h2 class="font-display font-semibold text-warm-900 dark:text-warm-50 text-lg leading-tight group-hover:text-amber-700 dark:group-hover:text-amber-300 transition-colors">
									{pet.name ?? 'Sin nombre'}
								</h2>
								<Badge.Root variant="amber" class="shrink-0 text-xs">
							...
							{#if pet.description}
								<p class="text-xs text-warm-500 dark:text-warm-400 line-clamp-2 leading-relaxed">{pet.description}</p>
							{/if}
							...
							<div class="mt-auto pt-3 border-t border-warm-100 dark:border-warm-700 flex items-center justify-between text-xs text-warm-400 dark:text-warm-500">
								<span class="flex items-center gap-1 truncate">
									...
									<span class="truncate">{pet.colonia ?? 'Ubicación desconocida'}</span>
								</span>
								<span class="shrink-0 ml-2">{timeAgo(pet.lastSeenAt)}</span>
							</div>
						</div>
					</a>
				{/each}
