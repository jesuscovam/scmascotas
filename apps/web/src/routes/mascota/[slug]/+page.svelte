<script lang="ts">
	...
	const whatsappUrl = $derived(
		pet.contactWhatsapp
			? `https://wa.me/${pet.contactWhatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola, vi el reporte de tu mascota en SC Pets: ${pet.name ?? 'sin nombre'}`)}`
			: null
	);
...
<div class="max-w-4xl mx-auto px-4 py-10">
	<a href="/" class="text-sm text-warm-500 hover:text-warm-700 transition-colors">← Volver al listado</a>

	<div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
		<!-- Photos -->
		<div class="flex flex-col gap-3">
			<div class="bg-warm-100 rounded-2xl overflow-hidden aspect-square flex items-center justify-center text-7xl">
				{#if primaryPhoto}
				...
				<div class="flex gap-2 overflow-x-auto pb-1">
					{#each pet.photos as photo, i (photo.id)}
						<button
							...
							class="shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors {activePhoto === i ? 'border-brand-600' : 'border-transparent'}"
						>
		...
		<div class="flex flex-col gap-6">
			<div>
				<div class="flex items-center gap-2 mb-2">
					<span class="bg-brand-50 text-brand-800 border border-brand-200 text-xs font-semibold px-3 py-1 rounded-full">
						{speciesLabel[pet.type] ?? pet.type}
					</span>
					<span class="bg-green-50 text-green-700 border border-green-200 text-xs font-semibold px-3 py-1 rounded-full">
						Buscando
					</span>
				</div>
	...
	<div class="mt-12 bg-white border border-warm-200 rounded-2xl p-6">
		...
		<p class="text-warm-500 text-sm">
			La función de reportar avistamientos estará disponible próximamente. Por ahora, contacta directamente al dueño.
		</p>
	</div>
</div>
